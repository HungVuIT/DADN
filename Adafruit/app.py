...
def room1():
    ...
    temp = Get_Temp()
    humid= Get_Humid()
    return render_template('user.html', room=data, led=led, fan=fan, temp=temp, humid=humid)
...
def showHistory():
    ...
    sql3 = 'select temp from Room where Id = 1;'
    data3 = getQuery(sql3).fetchall()
    if request.method == "POST":
        temp_input = request.form['input_temp']
        sql4 = 'update Room set temp =' + temp_input + 'where Id = 1;'
        executeQuery(sql4)
